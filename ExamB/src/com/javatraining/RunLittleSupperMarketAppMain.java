...
import com.javatraining.person.Customer;
import com.javatraining.supermarket.LittleSuperMarket;
import com.javatraining.supermarket.Merchandise;

import java.util.Scanner;

public class RunLittleSupperMarketAppMain {
    public static void main(String[] args) {
        // 1.创建一个小超市类 littleSuperMarket
        LittleSuperMarket littleSuperMarket = new LittleSuperMarket();
        // 2.依次给超市的名字，地址，停车位赋值 ("ADV 小超市"， “华佗路168号”，“100”)
        littleSuperMarket.superMarketName = "ADV 小超市";
        littleSuperMarket.address = "华佗路168号";
        littleSuperMarket.parkingCount = 100;
        // 3.给超市200种商品
        littleSuperMarket.merchandises = new Merchandise[200];
        // 4.初始化统计用的数组 merchandiseSold
        littleSuperMarket.merchandiseSold = new int[littleSuperMarket.merchandises.length];
        // 5.为了使用方便，创建一个商品数组引用all，和littleSuperMarket.merchandises指向同一个数组对象
        Merchandise[] all = littleSuperMarket.merchandises;
        // 6.遍历并200种商品
        for (int i = 0; i < all.length; i++) {
            // 7.创建并给商品的属性赋值 (例如，名称: 商品1，数量: 100，采购价格：100元以内的随机值，售价：采购价格 * 1~2的随机值，ID：ID1 )
            Merchandise m = new Merchandise();
            m.name = "商品1";
            m.count = 100;
            m.purchasePrice = 100 * Math.random();
            m.soldPrice = m.purchasePrice * (1 + Math.random());
            m.id = "ID1";
            // 8.用创建的商品，给商品数组的第i个引用赋值，all和小超市的商品数组引用指向的是同一个数组对象
            all[i] = m;
        }
